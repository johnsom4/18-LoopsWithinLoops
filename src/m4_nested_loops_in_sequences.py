...
def run_test_sum_numbers():
    ...
    # Test 3:
    expected = 1076
    answer = sum_numbers([(4, 41, 44), (160, 510), [20, 232, 31, 34]])
    print('Expected and actual are:', expected, answer)
...
def print_characters(sequence_of_strings):
    ...
    for k in range(len(sequence_of_strings)):
        subboi = sequence_of_strings[k]
        for j in range(len(subboi)):
                print(subboi[j])
...
def print_characters_slanted(sequence_of_strings):
    ...
    boi = ''
    for k in range(len(sequence_of_strings)):
        submarine = sequence_of_strings[k]
        for j in range(len(submarine)):
            print((' '*j) + submarine[j])
